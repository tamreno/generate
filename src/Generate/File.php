...
class Generate_File
{
     ...
     * 
     * @param string $delimiter to indicate how the fields will be delimited. 
    ...
    {
        //If there is no delimiter defined (csv file type) then use ',' as default.
        $this->_delimiter ?? ',';
        $_content = '';
        if(!empty($this->_header))
        {
            $_content .= implode($this->_delimiter, $this->_header[0]) . "\r\n";
        }
        foreach($this->_rows as $row) 
        {
